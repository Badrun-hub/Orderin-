...
export default function ManajemenKasir() {
  ...
  if (loading) return <div className="text-on-surface p-10 text-center font-bold">Memuat Data Pekerja...</div>
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
             {kasirList.map((kasir) => (
               ...
               >
                 {kasir.pin && (
                   <div className="absolute top-4 right-4 bg-on-surface/10 px-2 text-[10px] rounded text-on-surface-variant font-mono tracking-widest border border-outline-variant">PIN: {kasir.pin}</div>
                 )}
             ...
             {kasirList.length === 0 && (
                <div className="p-6 text-center border border-dashed border-outline-variant rounded-2xl">
                   <p className="text-sm text-on-surface-variant mb-4">Tidak ada data Pekerja Staf.</p>
                </div>
             )}
          </div>
                ...
                <div className="flex justify-between items-start mb-8 pb-6 border-b border-outline-variant">
                  <div>
                     <div className="flex items-center gap-3 mb-1">
                        ...
                        {selectedKasir.pin && <span className="bg-on-surface/10 text-on-surface font-mono px-2 py-1 rounded text-xs border border-outline-variant">PIN: {selectedKasir.pin}</span>}
                     </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button 
                      ...
                      className="px-4 py-2 bg-surface-container-highest rounded-lg text-xs font-bold hover:bg-on-surface/10 transition-colors"
                    >
                    ...
                    <tbody>
                      {shifts.filter(s => s.kasir_id === selectedKasir.id).length > 0 ? (
                        shifts.filter(s => s.kasir_id === selectedKasir.id).map(shift => (
                          <tr key={shift.id} className="border-b border-outline-variant hover:bg-on-surface/10">
      ...
      {/* Add Modal */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-on-surface/50 backdrop-blur-sm flex items-center justify-center p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-surface-container-low w-full max-w-sm rounded-[2rem] p-6 ghost-border">
             ...
             <form onSubmit={handleAddKasir} className="space-y-4">
               ...
               <div>
                 ...
                 <p className="text-[10px] text-on-surface-variant mt-2 text-center">PIN Rahasia ini akan digunakan untuk login di Portal Kasir.</p>
               </div>
               
               <div className="flex gap-3 mt-8">
                 <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 py-3 bg-surface-container-highest text-sm font-bold uppercase rounded-xl hover:bg-on-surface/10 transition-colors">Batal</button>
