  ...
  if (!currentOrderId && cart.length === 0) {
    return (
      <div className="bg-surface text-on-surface min-h-screen flex flex-col items-center justify-center p-6 text-center">
        <span className="material-symbols-outlined text-surface-variant text-6xl mb-4 text-on-surface-variant font-black">warning</span>
  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen pb-32 pt-2 relative overflow-hidden font-body">
      ...
      <header className="bg-surface-container-low/80 backdrop-blur-md sticky top-0 w-full z-50 px-6 py-4 flex items-center justify-between border-b border-outline-variant">
        ...
        <h1 className="text-lg font-bold font-headline tracking-tighter text-on-surface">Konfirmasi Transfer</h1>
      ...
      <main className="pt-6 px-6 max-w-xl mx-auto space-y-8 relative z-10">
        ...
        <section className="bg-surface-container-high p-8 rounded-[2rem] ghost-border text-center relative overflow-hidden">
          ...
          <p className="text-[10px] uppercase font-black text-on-surface-variant mb-2 font-label tracking-widest text-center">Silakan Transfer Sebesar</p>
          <h2 className="text-4xl font-black font-headline text-primary mb-8 tracking-tighter">{formatRupiah(total)}</h2>
          
          {isQris ? (
            <div className="flex flex-col items-center gap-4 bg-on-surface text-surface p-6 rounded-[2rem] mx-auto max-w-[260px] shadow-2xl border-4 border-emerald-500/10">
            ...
            <div className="flex flex-col items-center gap-2 bg-surface-container p-6 rounded-[1.5rem] text-left mx-auto border border-outline-variant">
              ...
              <div className="flex gap-4 items-center bg-surface-container-highest p-4 rounded-xl mt-1 w-full justify-between cursor-pointer group hover:bg-on-surface/10 transition-all">
                <span className="font-headline font-black text-xl tracking-[0.15em] text-primary">8803 2199 4443 11</span>
                <span className="material-symbols-outlined text-on-surface-variant group-hover:text-primary transition-colors text-lg font-black">content_copy</span>
              </div>
              <p className="text-[10px] text-on-surface-variant font-bold uppercase mt-2 px-1">Atas Nama: PT {cafeName || 'Orderin'} Indonesia Makmur</p>
            </div>
          )}
        </section>
            ...
            {!previewFile ? (
              <label className="border-2 border-dashed border-slate-800 rounded-[2rem] h-56 flex flex-col items-center justify-center p-6 text-on-surface-variant hover:bg-surface-container-high hover:border-primary/50 transition-all cursor-pointer group">
                <div className="w-16 h-16 rounded-full bg-surface-container-highest flex items-center justify-center mb-4 group-hover:bg-primary/20 transition-all">
                   <span className="material-symbols-outlined text-3xl text-on-surface-variant group-hover:text-primary transition-colors font-black">photo_camera</span>
                </div>
                <span className="font-bold mb-1 text-on-surface">Pilih Foto Bukti</span>
                <span className="text-[10px] uppercase font-black tracking-widest text-on-surface-variant">JPG, PNG, atau Screenshot</span>
