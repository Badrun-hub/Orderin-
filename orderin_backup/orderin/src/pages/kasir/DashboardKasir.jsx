...
export default function DashboardKasir() {
          ...
          <div key={table.id} className="bg-surface-container/50 rounded-2xl p-5 relative overflow-hidden group hover:bg-surface-container transition-colors border border-dashed border-outline-variant cursor-pointer">
            <div className="flex justify-between items-start mb-4" onClick={() => navigate(`/kasir/order/${table.id}`)}>
              <div>
                ...
                <h4 className="text-sm font-bold text-on-surface-variant mt-1">{table.lokasi || name}</h4>
              </div>
              <span className="bg-surface-container-highest/50 text-on-surface-variant px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider">Kosong</span>
            </div>
            <div className="mt-8">
              <button onClick={(e) => { e.stopPropagation(); actionDudukkanTamu(table.id) }} className="w-full py-3 bg-transparent text-on-surface-variant border border-slate-700 rounded-xl text-xs font-bold uppercase tracking-widest hover:border-primary hover:text-primary transition-all">Dudukkan Tamu</button>
            </div>
          </div>
        )
      default: return null
  ...
  return (
    <div className="text-on-surface min-h-screen pb-24 bg-surface font-body">
      {/* TopAppBar */}
      <header className="fixed top-0 w-full z-50 bg-surface-container-low/80 backdrop-blur-md flex justify-between items-center px-6 py-4 border-b border-outline-variant">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-on-surface-variant text-[10px] font-label uppercase tracking-widest mt-1">Toko Utama (LIVE)</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:flex flex-col items-end">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-on-surface-variant uppercase tracking-wider">Clocked In • {currentTime.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
            </div>
          </div>
          <button 
            onClick={async () => {
      ...
      <main className="pt-24 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex flex-col">
            ...
            <span className="text-xs text-on-surface-variant uppercase tracking-[0.1em] mt-1 font-label">Semua Lantai & Area</span>
          </div>
          <div className="flex gap-2">
        ...
        {loading ? (
             <div className="py-20 text-center text-on-surface-variant font-bold">Sinkronisasi Database Live...</div>
          ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {tables.map(renderTableCard)}
            {tables.length === 0 && (
                <div className="col-span-full py-16 text-center text-on-surface-variant border border-dashed border-outline-variant rounded-[2rem]">
      ...
      <nav className="fixed bottom-0 left-0 w-full z-50 bg-surface-container-low/90 backdrop-blur-xl flex justify-around items-center p-3 pb-safe border-t border-outline-variant">
        <button onClick={() => alert("Daftar pesanan aktif dapat dilihat langsung di masing-masing kartu meja di atas.")} className="flex flex-col items-center justify-center text-on-surface-variant hover:text-[#4edea3] transition-colors w-16">
