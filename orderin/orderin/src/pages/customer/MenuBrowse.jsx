...
export default function MenuBrowse() {
  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen pb-32 pt-2 relative overflow-hidden">
      ...
      <header className="bg-[#091421]/80 backdrop-blur-md fixed top-0 w-full z-50 flex justify-between items-center px-6 py-4 border-b border-outline-variant">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-slate-400 font-label uppercase tracking-[0.15em] mt-0.5">Meja {tableNo} ({lokasi}) • {customerName}</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <main className="pt-24 px-6 max-w-5xl mx-auto relative z-10">
        ...
        <section className="mb-8">
          <div className="relative flex items-center group">
            <span className="material-symbols-outlined absolute left-5 text-slate-500 group-focus-within:text-primary transition-colors">search</span>
            <input 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-surface-container-high border border-outline-variant rounded-[1.5rem] py-5 pl-14 pr-6 text-on-surface placeholder:text-slate-500 focus:ring-2 focus:ring-primary/20 transition-all outline-none shadow-xl shadow-black/20 font-medium" 
            ...
            <button 
              ...
              className={`px-6 py-3 rounded-2xl font-label text-xs uppercase tracking-widest whitespace-nowrap border transition-all duration-300 ${
                activeCategory === 'Semua' 
                  ...
                  : 'bg-surface-container border-outline-variant text-slate-400 hover:border-slate-700'
              }`}
            >
              Semua Menu
            </button>
            ...
            filteredMenus.map(item => (
              <div key={item.id} className="group relative flex flex-col bg-surface-container rounded-[2rem] overflow-hidden transition-all duration-500 border border-outline-variant hover:translate-y-[-8px] hover:shadow-2xl hover:shadow-primary/10">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <p className="text-sm text-slate-400 mb-6 font-medium line-clamp-2">{item.deskripsi}</p>
                  <div className="mt-auto flex justify-between items-end">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">{item.categories?.nama}</span>
          ...
          >
            <div className="flex items-center gap-4 py-2">
               ...
               <div className="flex flex-col items-start leading-tight">
                  ...
                  <span className="text-xl font-headline font-black text-white">{formatRupiah(totalPrice)}</span>
               </div>
            </div>
            <div className="bg-white px-8 py-4 rounded-[2.2rem] flex items-center gap-2 text-[#003824] group-hover:bg-emerald-50 transition-colors">
