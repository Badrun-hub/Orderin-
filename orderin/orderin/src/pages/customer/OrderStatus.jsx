...
export default function OrderStatus() {
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      case 'ordered': return { label: 'Menunggu Konfirmasi', color: 'bg-slate-500/10 text-slate-400', icon: 'pending_actions', title: 'Pesanan Terkirim' }
      ...
      default: return { label: 'Status Unkown', color: 'bg-slate-500', icon: 'help', title: 'Memproses...' }
    }
  }

  if (loading && currentOrderId) {
    return <div className="min-h-screen bg-surface flex flex-col items-center justify-center text-slate-500 font-bold">Menghubungkan ke Dapur...</div>
  }
  ...
  if (!currentOrderId) {
    return (
       <div className="bg-surface text-on-surface min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-sm text-slate-400 mb-8 max-w-xs">Scan QR di meja Anda kembali untuk mulai memesan hidangan lezat.</p>
  ...
  return (
    <div className="bg-surface text-on-surface font-headline min-h-screen pb-32 pt-2 relative overflow-hidden">
      {/* Top Navigation Bar */}
      <nav className="fixed top-0 w-full z-50 bg-[#091421]/80 backdrop-blur-md flex justify-between items-center px-6 py-4 border-b border-outline-variant">
        ...
        <div className="flex items-center gap-2">
           <span className="text-[10px] uppercase font-black tracking-widest text-slate-500 border border-outline-variant px-2 py-1 rounded">Live Track</span>
        </div>
      </nav>
      ...
      <main className="pt-24 px-6 max-w-2xl mx-auto space-y-8 relative z-10">
        ...
        <section className="relative overflow-hidden rounded-[2.5rem] bg-surface-container-high p-8 ghost-border shadow-2xl">
          <div className="relative z-10 flex flex-col items-center text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-slate-400 font-medium text-sm font-body">Meja {tableNo} ({lokasi}) • {order?.customer_name}</p>
            </div>
        ...
        <div className="bg-surface-container rounded-[2rem] p-6 ghost-border">
            ...
            {items.map((item) => (
              <div key={item.id} className="flex items-center justify-between group">
                <div className="flex items-center gap-4">
                  <div className="h-10 w-10 rounded-xl bg-surface-container-highest flex items-center justify-center text-slate-500 font-black border border-outline-variant text-xs">
                    {item.qty}x
                  </div>
                  <div>
                    <h3 className="font-bold text-on-surface text-sm">{item.nama_menu}</h3>
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest leading-none mt-1">{formatRupiah(item.harga)}</p>
                  </div>
                </div>
          ...
          <div className="mt-8 pt-6 border-t border-outline-variant flex justify-between items-center">
             <span className="text-xs uppercase font-black tracking-widest text-slate-500">Total Tagihan</span>
        ...
        <div className="bg-surface-container-low rounded-[1.5rem] p-6 flex flex-col sm:flex-row gap-4 sm:items-center justify-between border border-outline-variant">
          <div className="flex items-center gap-3">
             ...
             <div>
                <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest leading-none mb-1">Metode Bayar</p>
                <p className="font-bold text-on-surface uppercase text-sm">{order?.payment_method?.toUpperCase() || 'TUNAI'}</p>
             </div>
          </div>
          <p className="text-[10px] text-slate-600 font-medium font-body uppercase tracking-wider">Order ID: {order?.id?.substring(0,8)}</p>
        </div>
