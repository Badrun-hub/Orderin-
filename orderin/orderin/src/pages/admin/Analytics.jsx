...
export default function Analytics() {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-[#091421] p-4 rounded-xl border border-outline-variant shadow-2xl">
  ...
  if (loading) return <div className="text-white p-10 font-bold text-center">Menghitung Data Laporan...</div>
    ...
    <>
      ...
      <div className="flex flex-col gap-8 print:pt-4">
        ...
        <div className="hidden print:block mb-8 text-center border-b border-black pb-4">
          ...
          <p className="text-sm text-slate-600">Dicetak pada: {new Date().toLocaleString('id-ID')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-surface-container-highest p-6 rounded-2xl print:bg-white print:border print:border-slate-200">
          ...
          <div className="bg-surface-container-highest p-6 rounded-2xl sm:col-span-2 lg:col-span-3 print:bg-white print:border print:border-slate-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-surface-container p-6 rounded-3xl ghost-border print:bg-white print:border-none print:shadow-none">
             <div className="mb-6">
                <h2 className="text-xl font-bold font-headline print:text-black">Tren Pendapatan</h2>
                <p className="text-xs text-on-surface-variant print:text-slate-600">Pergerakan transaksi harian periode aktif</p>
             </div>
             ...
             <div className="w-full h-72">
               {totalOrdersQty > 0 ? (
                 ...
                 <div className="w-full h-full flex items-center justify-center text-slate-500 text-sm">Masih belum ada data pesanan (Real DB).</div>
               )}
             </div>
          </div>

          <div className="lg:col-span-1 bg-surface-container p-6 rounded-3xl ghost-border flex flex-col print:bg-white print:border-none print:shadow-none">
             <div className="mb-6">
                <h2 className="text-xl font-bold font-headline print:text-black">Menu Terlaris</h2>
                <p className="text-xs text-on-surface-variant print:text-slate-600">Berdasarkan kuantitas asli yang dipesan</p>
             </div>
             ...
             <div className="flex-1 w-full flex items-center justify-center -ml-4">
               {topMenus.length > 0 ? (
                 ...
                 <div className="text-slate-500 text-sm text-center">Belum ada pesanan laku.</div>
               )}
             </div>
          </div>
        </div>
