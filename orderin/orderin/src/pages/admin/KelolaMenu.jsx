...
export default function KelolaMenu() {
  ...
  if (loading && menus.length === 0) return <div className="text-white p-20 text-center font-bold">Memuat Supabase Menu...</div>
  ...
  return (
    <>
            ...
            <ul className="flex flex-col gap-2">
              ...
              {categories.map(cat => (
                <li 
                  ...
                  className={`px-4 py-3 rounded-xl text-sm font-bold cursor-pointer transition-colors ${activeTab === cat.nama ? 'bg-primary text-on-primary' : 'hover:bg-white/5 text-on-surface-variant'}`}
                >
                  {cat.nama}
                </li>
              ))}
              ...
              <div key={menu.id} className={`bg-surface-container rounded-[2rem] p-4 flex gap-5 transition-all ${!menu.is_available ? 'opacity-60' : ''} hover:shadow-xl hover:shadow-primary/5 group`}>
                ...
                <div className="w-24 h-24 rounded-2xl overflow-hidden bg-surface-container-highest flex-shrink-0 relative">
                   {menu.foto_url ? (
                     ...
                     <div className="w-full h-full flex flex-col items-center justify-center text-slate-600 bg-surface-container-highest">
                ...
                <div className="flex-grow flex flex-col">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <div className="flex gap-2 relative z-10">
                      <button onClick={(e) => { e.stopPropagation(); handleOpenEdit(menu); }} className="w-8 h-8 rounded-lg bg-white/5 text-slate-300 hover:text-white hover:bg-white/10 flex items-center justify-center transition-colors shadow-sm">
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-outline-variant">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <div className={`w-4 h-4 bg-white rounded-full transition-transform ${menu.is_available ? 'translate-x-5' : 'translate-x-0'}`}></div>
                      </button>
                    </div>
                  </div>
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 lg:p-6 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-surface-container-low w-full max-w-lg rounded-[2.5rem] ghost-border flex flex-col max-h-[90vh] overflow-hidden shadow-2xl relative">
             ...
             <div className="p-7 border-b border-outline-variant flex justify-between items-center">
               <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">Detail Produk di Katalog Online</p>
               </div>
               <button onClick={() => setShowModal(false)} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-white transition-all hover:bg-white/10 group">
             ...
             <div className="p-7 overflow-y-auto custom-scrollbar flex flex-col gap-8">
               ...
               <div className="flex flex-col items-center gap-4 pt-2">
                     ...
                     <div className="w-40 h-40 bg-surface-container rounded-[2rem] overflow-hidden border-2 border-dashed border-outline-variant flex items-center justify-center group-hover:border-primary/50 transition-all shadow-inner">
                        ...
                        {isUploading && (
                           <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-2 backdrop-blur-sm">
                              ...
                              <span className="text-[9px] text-white font-black uppercase tracking-widest">Grouting...</span>
                           </div>
                        )}
                     </div>
                     <button 
                  ...
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest text-center">Rekomendasi Size: 800 x 800 Pixel</p>
               </div>

               <form id="menuForm" onSubmit={handleSave} className="flex flex-col gap-6">
                 <div>
                   <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-[0.15em] font-label px-1">Nama Hidangan</label>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                   <div>
                     <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-[0.15em] font-label px-1">Kategori</label>
                   ...
                   <div>
                     <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-[0.15em] font-label px-1">Harga (Rp)</label>
                 ...
                 <div>
                   <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-[0.15em] font-label px-1">Foto Menu (Upload atau URL)</label>
                   <div className="flex flex-col gap-3">
                      <div className="flex gap-2">
                        ...
                        <button 
                          type="button"
                          onClick={() => fileInputRef.current?.click()}
                          className="px-4 bg-surface-container-highest border border-outline-variant rounded-xl hover:bg-white/5 text-primary transition-all flex items-center gap-2"
                          title="Upload File"
                        >
                      ...
                      <p className="text-[9px] text-slate-600 px-1 italic">* Tempel URL gambar atau klik tombol Upload untuk memilih file.</p>
                   </div>
                 </div>

                 <div>
                   <label className="block text-[10px] font-black text-slate-500 mb-2 uppercase tracking-[0.15em] font-label px-1">Deskripsi & Bahan</label>
             ...
             <div className="p-7 border-t border-outline-variant flex gap-4 bg-surface-container-low/50">
               <button onClick={() => setShowModal(false)} className="flex-1 py-4 bg-surface-container-highest text-slate-300 text-xs font-black uppercase tracking-widest rounded-2xl hover:bg-white/5 transition-all text-center">Batal</button>
      ...
      >
        <form id="catForm" onSubmit={handleSaveCategory} className="pt-2">
          <p className="text-[10px] text-center text-slate-500 font-bold uppercase tracking-widest mb-4">Urutan Menu Akan Dikelompokkan Per Kategori</p>
          <input 
          ...
          <div className="flex gap-3">
             <button type="button" onClick={() => setCategoryModal(false)} className="flex-1 py-4 bg-surface-container-highest text-xs font-black uppercase tracking-widest rounded-2xl hover:bg-white/5 transition-colors">Batal</button>
